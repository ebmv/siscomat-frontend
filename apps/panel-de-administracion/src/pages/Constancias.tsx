...
import { faCheckCircle, faTriangleExclamation, faCircleInfo } from "@fortawesome/free-solid-svg-icons";
...
export const Constancias = () => {
  ...
  const [toastConfig, setToastConfig] = useState<{ mensaje: string; variant: "success" | "error" }>({ mensaje: "", variant: "success" });
  ...
  const handleCSVUpload = (file: File | null) => {
    ...
    if (!file.name.toLowerCase().endsWith(".csv")) {
      setErrorCSV("Archivo inválido. Se requiere estrictamente un archivo con extensión .csv.");
    ...
    reader.onload = (e) => {
      ...
      if (text.includes('')) {
        setErrorCSV("Codificación inválida detectada. El archivo contiene caracteres no reconocidos. Por favor, exporte o guarde su documento utilizando explícitamente la codificación 'UTF-8' en su procesador de hojas de cálculo.");
        setParticipantes([]);
        setArchivoSubido(null);
        return;
      }
      ...
      if (lineas.length < 2) {
        setErrorCSV("El archivo CSV carece de datos estructurales o está vacío.");
        setParticipantes([]);
        return;
      }
  ...
  const handleValidarCSV = async () => {
    ...
    try {
      ...
      const msg = error.response?.data?.error || "Error no controlado al validar archivo.";
      const detalle = error.response?.data?.detalle;
      setToastConfig({ mensaje: detalle ? `${msg} ERROR: ${detalle}` : msg, variant: "error" });
    } finally {
  ...
  const handleConfirmarGeneracion = async () => {
    ...
    try {
      ...
      setToastConfig({ mensaje: "Constancias procesadas y generadas exitosamente.", variant: "success" });
      setShowModal(false);
      setValidacionData(null);
      setSeReordeno(false);
      setArchivoSubido(null);
      ...
      const msg = error.response?.data?.error || "Error al generar el lote de constancias.";
      ...
      <Modal isOpen={showModal} onClose={handleCancelar}>
        ...
        <div className="flex flex-col gap-4 w-full px-2">
          ...
          <p className="label-small text-dark-3 text-center mt-1 mb-2">
            Presionar "Sí, generar" procesará las filas válidas y descartará los registros anómalos.
          </p>
          ...
          {validacionData?.errores && validacionData.errores.length > 0 && (
            <div className="mt-2 p-4 bg-error-primary/10 border border-error-primary/30 rounded-md text-sm text-error-primary max-h-60 overflow-y-auto text-left">
              <p className="font-bold mb-3">Registros no válidos ({validacionData.errores.length}):</p>
          ...
          <Card className="w-full lg:w-3/5 overflow-hidden shadow-card rounded-md border border-light-2 flex flex-col h-100 lg:h-full">
            {seReordeno && (
              <div className="bg-warning-subtle border-b border-warning-primary/20 py-3 px-4 flex flex-col items-center justify-center gap-1 text-warning-darker text-center">
                ...
                <p className="label-small text-dark-1">
                  La matriz ha sido reordenada según su nivel de validez. Verifique los registros anómalos.
                </p>
              </div>
            )}
            ...
            {participantes.length === 0 ? (
              <div className="w-full h-full flex items-center justify-center p-4">
                <p className="label-normal text-dark-3 text-center">Sube un archivo CSV para renderizar los registros</p>
              </div>
            ) : (
                    ...
                    {participantes.map((p, index) => {
                      ...
                      return (
                        <tr
                          ...
                          className={`cursor-pointer transition-colors ${errorInfo
            ...
            {loadingPreview && (
              <div className="absolute inset-0 flex items-center justify-center bg-white/50 z-10 backdrop-blur-sm">
                <div className="bg-white p-4 rounded-lg shadow-card">
                  <p className="label-large text-brand-primary">Renderizando documento PDF...</p>
                </div>
              </div>
            )}
